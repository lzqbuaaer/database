<template>
  <div>
    ...
    <div class="card" style="margin-bottom: 10px;">
      ...
      <el-input style="width: 300px; margin-right: 10px" v-model="data.username" placeholder="请输入编号查询" :prefix-icon="Search"/>
    ...
    <div class="card" style="margin-bottom: 10px">
      ...
      <el-table stripe :data="data.tableData">
        ...
        <el-table-column label="性别" prop="tsex"></el-table-column>
        <el-table-column label="系别" prop="tdept"></el-table-column>
        <el-table-column label="邮箱" prop="temail"></el-table-column>
    ...
    <el-dialog title="信息" width="40%" v-model="data.formVisible" :close-on-click-modal="false" destroy-on-close>
      <el-form :model="data.form" label-width="100px" style="padding-right: 50px">
        ...
        <el-form-item label="性别" prop="ssex">
          <el-input v-model="data.form.tsex" autocomplete="off" />
        </el-form-item>
...
const load=()=>{
  axios.get('http://localhost:9090/teacher/selectPage',{
    params:{
...
const save=()=>{
  ...
  axios.request({
    url:data.isUpdate? 'http://localhost:9090/teacher/updateByTNO':'http://localhost:9090/teacher/addTeacherInfo',
...
const handleDelete = (username) => {
  ...
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗?', '删除确认', { type: 'warning' }).then(res => {
    axios.delete('http://localhost:9090/teacher/deleteByTNO/'+username).then(res=>{
      if(res.data.code=== '200'){
        load();
        ElMessage.success('操作成功!');
      }else {
